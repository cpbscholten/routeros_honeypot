...
namespace
{
    ...
    void print_csv_without_message(int srcIP, uint16_t srcPort, int destIP, uint16_t destPort)
    {
      print_ip_from_int(srcIP);
      std::cout << ",";
      printf("%" PRIu16, srcPort);
      std::cout << ",";
      print_ip_from_int(destIP);
      std::cout << ",";
      printf("%" PRIu32, destPort);
      std::cout << ",";
    }
    ...
    switch(m_state) {
        ...
        case k_decrypt:
                ...
                p_length > 15 &&  memcmp(p_data, "GET /jsproxy/?%", 15) == 0)
            {
                ...
                print_csv_without_message(p_srcAddr, p_srcPort, p_destAddr, p_destPort);
