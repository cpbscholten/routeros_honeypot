...
void parse_string(std::string& p_data, boost::uint32_t srcAddress,
  boost::uint16_t srcPort, boost::uint32_t destAddress, boost::uint16_t destPort)
{
    while (p_data.size() > 6)
    {
        ...
        // escape the " character in the jsproxy serialized json data
        std::string jsproxy_json = msg.serialize_to_json();
        boost::replace_all(jsproxy_json, "\"", "\\\"");
        // print the json formatted as csv
        print_ip_from_int(srcAddress);
        std::cout << "," << srcPort << ",";
        print_ip_from_int(destAddress);
        std::cout << "," << destPort << ",";
        std::cout << "\"" << jsproxy_json << "\"" <<std::endl;
...
int main(int p_argc, const char** p_argv)
{
    ...
    while ((packet = pcap_next(handle, &header)) != NULL)
    {
        ...
        if (sourcePort == 8291)
        {
            ...
            parse_string(buffer[false], srcAddress, sourcePort, dstAddress, dstPort);
        }
        else
        {
            buffer[false].append((const char*)packet, length);
            parse_string(buffer[true], srcAddress, sourcePort, dstAddress, dstPort);
        }
    }
